...
def get_files():
    """
    READS ALL THE IMAGES FROM THE FOLDER AND RETURNS A DATAFRAME WITH THE IMAGE LOCATION AND LABELS
    THE LABELS ARE ENCODED USING SKLEARN PREPROCESSING LABEL ENCODER
    AS PER PROBLEM STATEMENT ONLY FIRST 14 CLASSES ARE ONLY USED FOR TRAINING, TESTING AND VALIDATION
    THE REMAINING CLASSES ARE KEPT AS ALTERNATE DATAFRAME FOR MAX SOFTMAX CHECK
    FUNCTION RETURN TRAIN, TEST, VALIDATION ALTERNATE DATAFRAMES AND LABEL ENCODERS 
    """
...
def load_data(dataframe):
    """
    LOADS THE IMAGES FROM THE DATAFRAME
    RESIZES AND PREPROCESSES THE IMAGES
     AND RETURNS THE IMAGES AND ENCODED CATEGORICAL LABELS
    """
    images, labels = [], []
